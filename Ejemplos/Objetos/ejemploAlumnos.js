...
function calcularMedias(arrayAlumnos) {
    arrayAlumnos.forEach(alumno => {
            ...
            let stringAlumno = JSON.stringify(Object.values(alumno)).split(',')
            stringAlumno = Object.values(alumno)
            console.log("String: "+ stringAlumno[1])
            
            for (let i = 6; i < stringAlumno.length - 2; i++) {
                suma += parseInt(stringAlumno[i])
                console.log(suma)
            }
            alumno.media = suma/6
    })
